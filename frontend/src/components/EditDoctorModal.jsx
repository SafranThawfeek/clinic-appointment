...
export default function EditDoctorModal({ isOpen, onClose, doctorData, onSuccess }) {
  ...
  useEffect(() => {
    if (doctorData) {
      setForm(doctorData);
      setPreview(doctorData.image ? `http://localhost:5000${doctorData.image}` : null);
    }
  }, [doctorData]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await updateDoctor(doctorData._id, form);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 overflow-y-auto">
      <div className="bg-white w-full max-w-5xl rounded-2xl shadow-lg p-8 relative">
        ...
        <h2 className="text-xl font-semibold mb-6 text-gray-800">Edit Doctor</h2>
