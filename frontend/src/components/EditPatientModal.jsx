...
export default function EditPatientModal({ isOpen, onClose, patientData, onSuccess }) {
  ...
  useEffect(() => {
    if (patientData) {
      ...
      setPreview(patientData.profileImage ? `http://localhost:5000${patientData.profileImage}` : null);
    }
  }, [patientData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add image if it's a File object (newly selected)
      if (form.profileImage instanceof File) {
        formData.append('profileImage', form.profileImage);
      }
      
      console.log('Updating patient with ID:', patientData._id);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 overflow-y-auto py-8">
      <div className="bg-white w-full max-w-2xl rounded-lg shadow-2xl p-6 relative">
        ...
        <h2 className="text-lg font-semibold mb-6 text-gray-800">Edit Patient</h2>
